...
from mesh.core.routing import EdgeInfo, LinkStateGossip, MeshGraph, decode_report, encode_report
...
def test_encode_decode_round_trip():
    entries = {"B": EdgeInfo(link_quality=0.75, recent_failures=2), "C": EdgeInfo(link_quality=1.0, recent_failures=0)}
...
def test_encode_caps_failures_at_255():
    payload = encode_report({"B": EdgeInfo(link_quality=1.0, recent_failures=1000)})
    decoded = decode_report(payload)
...
def test_receiving_report_merges_reported_edges_into_graph():
    ...
    payload = encode_report({"C": EdgeInfo(link_quality=0.5, recent_failures=1)})
...
def test_on_topology_change_fires_when_graph_actually_changes():
    ...
    payload = encode_report({"C": EdgeInfo(link_quality=1.0, recent_failures=0)})
    report = Packet(type=PacketType.LINK_STATE, priority=1, src="B", dst="A", ttl=1, payload=payload)
    gossip_a._on_receive("B", report.pack())
    assert len(fired) == 1
